...
class DistanceMetricClassifier(BaseEstimator, ClassifierMixin):
    ...
    def __init__(self, metric="euclidean", scale_std=True, n_feat_select=None,
                 canonical_stat="median",
                 calculate_kde=True, calculate_1d_dist=True, # Maybe change to False
                 n_jobs=-1):
        self.metric = metric
    ...
    def set_metric_fn(self):
        ...
        if not callable(self.metric) or isinstance(self.metric, str):
            ...
            elif hasattr(cd, self.metric):    
                self.metric_fn_ = getattr(cd, self.metric)
            else:
                raise ValueError(f"{metric} metric not found. Either pass a string of the name of a metric in scipy.spatial.distance or distances.Distance, or, pass a metric function directly.")
